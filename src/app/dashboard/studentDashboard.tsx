...
export default function StudentDashboard() {
  ...
  const fetchEnrolledBatches = async () => {
    // Fetch enrolled batches for the current student
    try {
      const response = await axios.get(
        `/api/batches/get?userIdString=${user?._id}`
      );

      setEnrolledBatches(response.data.data.active);
    } catch (error) {
      console.log(error);
    }
  };

  // TODO: Implement handleJoinBatch function
  const handleJoinBatch = async (values: z.infer<typeof formSchema>) => {
    // Handle joining a batch with the provided invite code
    try {
      ...
      <div className="w-full px-6 py-6">
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="rounded-lg border p-4">
            ...
            <div className="mb-12">
              <h2 className="text-2xl font-semibold mb-4">Enrolled Batches</h2>
